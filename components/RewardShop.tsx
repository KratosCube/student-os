...
export default function RewardShop({ totalLifetimeCredits }: { totalLifetimeCredits: number }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden flex flex-col w-full transition-colors">
      ...
      <div className="p-3 flex-1 overflow-y-auto max-h-[35vh] min-h-[200px]">
        <div className="space-y-2">
          {rewards.map(reward => (
            <div key={reward.id} className="group flex items-center justify-between p-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 border border-transparent hover:border-slate-100 dark:hover:border-slate-700 transition-all">
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium text-slate-700 dark:text-slate-200 text-sm truncate">{reward.name}</span>
              </div>
              ...
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => deleteReward(reward.id)} className="text-slate-300 hover:text-rose-500 dark:text-slate-600 dark:hover:text-rose-400 opacity-0 group-hover:opacity-100 transition-opacity p-1">
        ...
        {isAdding ? (
          <div className="mt-3 p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 animate-in slide-in-from-top-2">
            <input 
              autoFocus
              placeholder="Název..." 
              value={newRewardName}
              onChange={e => setNewRewardName(e.target.value)}
              className="w-full text-xs p-2 rounded border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 mb-2 focus:outline-none focus:border-indigo-500"
            />
            <div className="flex gap-2">
              <input 
                ...
                className="w-16 text-xs p-2 rounded border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 focus:outline-none focus:border-indigo-500"
              />
              <button onClick={handleAddReward} className="flex-1 bg-indigo-600 text-white text-xs font-bold rounded hover:bg-indigo-700">
                OK
              </button>
              <button onClick={() => setIsAdding(false)} className="px-3 bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-bold rounded hover:bg-slate-300 dark:hover:bg-slate-600">
                X
              </button>
            </div>
          </div>
          ...
          <button 
            ...
            className="w-full mt-3 py-2 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl text-slate-400 dark:text-slate-500 text-xs font-bold hover:border-indigo-300 dark:hover:border-indigo-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all flex items-center justify-center gap-2"
          >
